...
CREATE OR REPLACE TABLE c_hive.db01.t_unnested_sales (
    `invoicenumber`        STRING
    ,`store_id`            STRING
    ,`category`            STRING
    ,`brand`               STRING
...
SET 'pipeline.name' = 'SalesUnnested Basket - (t_unnested_sales) -> Kafka (avro_unnested_sales)';

INSERT INTO c_hive.db01.t_unnested_sales
  SELECT
...
CREATE TABLE c_paimon.dev.t_salespayments WITH (
    'file.format' = 'avro'
  ) AS SELECT 
     `invoiceNumber`
    ,`payDateTime_Ltz`
...
-- Just because we can we're going to do a seperate create table and then a insert into table.
-- Because we define a Primary Key you will notice a index folder in the table folder on s3.
SET 'pipeline.name' = 'SalesUnnested Basket (t_unnested_sales) -> Paimon (c_paimon.dev.t_unnested_sales)';

CREATE OR REPLACE TABLE c_paimon.dev.t_unnested_sales (
     `invoicenumber`       STRING
    ,`store_id`            STRING
    ,`category`            STRING
    ,`brand`               STRING
...
INSERT INTO c_paimon.dev.t_unnested_sales
  SELECT
   `invoicenumber`
  ,`store_id`
  ,`category`
  ...
  ,`saleDateTime_Ltz`
  ,`saleTimestamp_Epoc`
  FROM c_hive.db01.t_unnested_sales;
